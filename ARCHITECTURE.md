...
The [read-only Story World index](docs/story-world-index.md) is an in-memory projection. It is built from Markdown metadata at plugin startup, receives a layout-ready fallback rebuild, and reconciles whenever Obsidian reports a metadata batch resolved, including imports and external sync after startup. Metadata changes, creation, deletion and rename events also update individual paths, with a deferred cache read for new or changed Markdown notes. Settled reads invalidate dependent review projections and refresh index consumers; sidebar renders do not rebuild the index.
...
Project readiness is an immutable presentation projection, not authority. It collects the recognised manuscript library, invokes the same validated `ManuscriptPreparationPlan` used by #91 for each Book, reads the existing Story World index/review summary, and observes the already-loaded editorial store. Mapping, rendering, local invitation state, and command routing are separate. The projection is never persisted; manual refresh rebuilds it, and Book-specific migration routes to the existing preparation preview.
...
- Treating a resolved canonical Scene `location` link to an indexed Location as derived context
- Preserving existing manuscript metadata, free-text locations and unresolved/non-Location links
...
`location` is the sole canonical Scene location property. Chapter Context may write a canonical full-path wikilink selected only from indexed `world_entity: location` entities. Existing free text, unresolved links and links to non-Location entities remain preserved. A resolved indexed Location contributes automatically to derived context without being copied into `world_context`.

The derived context set combines resolved POV, resolved Location and explicit entries, and removes duplicates by resolved vault path. Alias and path variants may therefore identify one entity without creating duplicate authority.
