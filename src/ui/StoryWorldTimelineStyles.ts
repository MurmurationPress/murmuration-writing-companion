...
.mwc-event-scene-legend { margin: 16px 0; color: var(--text-muted); }
...
.mwc-event-scene-axis::before { content: ""; position: absolute; top: 0; bottom: 0; left: 7px; width: 3px; background: var(--interactive-accent); opacity: 0.55; }
...
.mwc-event-scene-scene-node { min-width: 0; padding: 10px 12px; border: 1px solid var(--background-modifier-border); border-radius: var(--radius-m); background: var(--background-secondary); transition: border-color 120ms ease, box-shadow 120ms ease, opacity 120ms ease; }
...
.mwc-event-scene-scene-node.is-unresolved { border-style: dashed; border-color: var(--color-orange); }
...
.mwc-timeline-assertion { margin-top: 8px; padding: 9px 11px; border: 1px solid var(--background-modifier-border); border-radius: var(--radius-m); }
.mwc-timeline-assertion.has-conflict { border-color: var(--color-orange); }
