...
import { isWikilinkActivationKey, presentWikilinkValues } from "../story-world/WikilinkPresentation";
...
export function renderWikilinkValues(
  ...
  values.forEach((item, index) => {
    ...
    if (target instanceof TFile) {
      ...
      link.onclick = (event) => { event.preventDefault(); void app.workspace.openLinkText(item.authored, sourcePath); };
      link.onkeydown = (event) => {
        if (!isWikilinkActivationKey(event.key)) return;
        event.preventDefault(); void app.workspace.openLinkText(item.authored, sourcePath);
      };
      component.registerDomEvent(link, "auxclick", (event) => {
        if (event.button === 1) { event.preventDefault(); void app.workspace.openLinkText(item.authored, sourcePath, true); }
      });
    } else {
