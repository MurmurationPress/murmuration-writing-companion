...
An indexed POV Profile may be a Book-scoped delta when it explicitly links its parent through `pov_extends` and its applicable Book through `world_scope`. After resolving the durable chain, Chapter Context discovers children from the existing Story World index, appends the single child matching the authoritative owning Book, and may repeat for another explicit descendant. Zero matches leave the chain unchanged. Multiple matching siblings are ambiguous and none is selected. Folders, filenames and manuscript order have no role in inheritance or applicability. The authoring templates create editable Markdown prompts only and do not make profile prose a rigid schema.
